...
old.clin$bcr_patient_barcode <- toupper(old.clin$bcr_patient_barcode)
colnames(old.clin)[1] <- 'submitter_id'
clinic <- merge(clinic, old.clin[, c('submitter_id', 'age_at_initial_pathologic_diagnosis')], by='submitter_id')
write.table(clinic, file.path(data.dir, 'clinical/clinical.tsv'), sep="\t", quote = F)
```
...
age_sum <- clinic %>%
  ...
  summarise(mean.age = mean(age_at_initial_pathologic_diagnosis, na.rm = T))
